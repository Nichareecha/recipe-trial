...
class ResultPage extends StatelessWidget {
  ...
  List<String> getRecipes() {
    ...
    if (dietaryRestrictions.contains('Vegetarian')) {
      recipes = recipes.where((recipe) => !recipe.contains('Bolognese')).toList();
    }
    ...
    if (dietaryRestrictions.contains('Halal')) {
      recipes = recipes.where((recipe) => !(recipe.contains('Pizza') && recipe.contains('Bolognese'))).toList();
    }
          ...
          children: [
            ...
            for (String recipe in recipes)
             ...
             // Navigate to a new page to show details of the selected recipe
              ...
              context,
              MaterialPageRoute(
              builder: (context) => RecipeDetailsPage(recipe, recipeDetails[recipe] ?? 'No details available'), // Pass the selected recipe and details here
             ),
           );
         },
             child: Text(recipe),
